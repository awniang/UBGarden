...
public final class SpriteFactory {
    ...
    public static Sprite create(Pane layer, GameObject gameObject) {
        ...
        if (gameObject instanceof Door door) {
            ImageResourceFactory fact = ImageResourceFactory.getInstance();
            Image image;
            if (door.isOpen()) {
                image = door.getTargetLevel() > door.getPosition().level()
                        ? fact.get(DOOR_OPENED_PLUS)
                        : fact.get(DOOR_OPENED_MINUS);
            } else {
                image = fact.get(DOOR_CLOSED_PLUS);
            }

            return new Sprite(layer, image, gameObject) {
                @Override
                public void updateImage() {
                    Door d = (Door) getGameObject();
                    Image newImage;
                    if (d.isOpen()) {
                        newImage = d.getTargetLevel() > d.getPosition().level()
                                ? ImageResourceFactory.getInstance().get(DOOR_OPENED_PLUS)
                                : ImageResourceFactory.getInstance().get(DOOR_OPENED_MINUS);
                    } else {
                        newImage = ImageResourceFactory.getInstance().get(DOOR_CLOSED_PLUS);
                    }
                    setImage(newImage);
                }
            };
